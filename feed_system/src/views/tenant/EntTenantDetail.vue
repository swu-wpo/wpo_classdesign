  ...
  <el-tabs v-model="activeName" @tab-click="handleClick">
    <el-tab-pane label="租户详情" name='first'>
      <div>
        <h5>企业信息</h5>
        <el-row>
          <el-col :span="8"><div>企业名称：{{enterpriseInfo.base.enterpriseName}}</div></el-col>
          <el-col :span="8"><div>企业简称：{{enterpriseInfo.detail.abbreviation}}</div></el-col>
          <el-col :span="8"><div>企业规模：{{enterpriseInfo.detail.scaleMember | formatScaleMember}}</div></el-col>
        </el-row>
        <el-row>
          <el-col :span="8"><div>公司性质：{{enterpriseInfo.base.enterpriseType | formatEnterpriseType}}</div></el-col>
        ...
        <el-row>
          <el-col :span="8"><div>钉钉：{{enterpriseInfo.dd}}</div></el-col>
          <el-col :span="8"><div>所属官网：{{enterpriseInfo.gw}}</div></el-col>
          <el-col :span="8"><div>传真：{{enterpriseInfo.cz}}</div></el-col>
        </el-row>
         <el-row>
          <el-col :span="8"><div>企业微信：{{enterpriseInfo.wx}}</div></el-col>
          <el-col :span="8"><div>企业介绍：{{enterpriseInfo.js}}</div></el-col>
          <el-col :span="8"><div></div></el-col>
        </el-row>
        <h5>管理员信息</h5>
        <el-row>
          <el-col :span="8"><div>姓名：{{userDetail.userName}}</div></el-col>
          <el-col :span="8"><div>身份证号码：{{userDetail.sex}}</div></el-col>
          <el-col :span="8"><div>电话号码：{{userDetail.birthday}}</div></el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    getEnterpriseInfo (tenantId) {
      enterpriseApi.getEnterpriseByTenantId(tenantId)
        .then((res) => {
          console.log(res)
  ...
  created () {
    ...
    this.getEnterpriseInfo(this.$route.query.tenantId)
  },
  filters: {
    ...
    formatEnterpriseType: function (val) {
      if (val === 1) {
        return '企业'
      }
      if (val === 2) {
    ...
    formatScaleMember: function (val) {
      if (val === 1) {
        return '1-1000'
      }
      if (val === 2) {
